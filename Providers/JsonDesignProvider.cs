...
namespace KitchenDesignManager.Providers
{
    public class JsonDesignProvider : IDesignProvider
    {
        ...
        {
            Folder = Application.persistentDataPath + "/UserData/KitchenDesignSelector";
            ...
            if (!File.Exists(path))
            {
                ...
                File.WriteAllText(path, JsonConvert.SerializeObject(designs));
            }
        }
        ...
        public Dictionary<string, string> GetDesigns()
        {
            ...
            {
                Debug.Log(Mod.MOD_NAME + " - Loading file: " + file);
